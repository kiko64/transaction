...
//        imgList.add('https://images.unsplash.com/photo-1520342868574-5fa3804e551c?ixlib=rb-0.3.5&ixid=eyJhcHBfaWQiOjEyMDd9&s=6ff92caffcdd63681a35134a6770ed3b&auto=format&fit=crop&w=1951&q=80');
        ...
        if ( _seguimiento != 12601 ) {  // Solo en peparación se puede cambiar
          print('U P D A T E ...' + widget.actual.seguimiento.toString() );
          _isVisible = false;
        }
        _fecha.text = widget.actual.fecha;
//        imgList.add('https://images.unsplash.com/photo-1520342868574-5fa3804e551c?ixlib=rb-0.3.5&ixid=eyJhcHBfaWQiOjEyMDd9&s=6ff92caffcdd63681a35134a6770ed3b&auto=format&fit=crop&w=1951&q=80');
  ...
  Future<void> _showMyDialog() async {
    return showDialog<void>(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: const Text('Desea eliminar el soporte actual?', style: TextStyle(color: Colors.black87, fontSize: 15),),
          content: SingleChildScrollView(
            child: ListBody(
              children: const <Widget>[
                Text(' '),
                Text('Borrar este soporte', style: TextStyle(fontSize: 14),),
              ],
            ),
          ),
          actions: <Widget>[
            TextButton(
              child: const Text('CANCELAR', style: TextStyle(color: Colors.teal, fontSize: 14), ),
              onPressed: () {
            ...
            TextButton(
              ...
              onPressed: () {
                _showToastOk(1, 'Soporte No. ${(_current+1).toString()} eliminado' );
...
//            Image.network(item, fit: BoxFit.cover, width: 1000.0),
          child: Image.file(File(item.toString()), fit: BoxFit.cover, width: 1000.0),
          onDoubleTap: () {
            _showToastOk(1, 'onDoubleTap No. ${(_current+1).toString()}' );
//            imgList.removeAt(_current);
//            _showMyDialog();
          },
          onLongPress : () {
            _showToastOk(1, 'onLongPress No. ${(_current+1).toString()}' );
//            imgList.removeAt(_current);
//            _showMyDialog();
          },
      ...
      if (_cambio) {
        ...
        if (widget.actual.ejecutar == 0 )
          if ( _agenda > 0 ) {
            ...
            var newData = {"fecha": _fecha.text, "usuario": 'Ocobo',
                "seguimiento": _seguimiento, "agenda": _agenda, "documento": _documento,
                "cuenta": _cuenta, "valor": int.parse(_valor.text.replaceAll(",", "")),
                "observacion": _observacion.text, "registro": '0', "mascara": _mascara,
                "archivo0": archivo0, "archivo1": archivo1, "archivo2": archivo2, "archivo3": archivo3};
...
//            await this.handler.insertUsingHelperMySql(newData);  // MySql
          ...
          var newData = {"ejecutar": widget.actual.ejecutar, "fecha": _fecha.text,
            "seguimiento": _seguimiento, "agenda": _agenda, "documento": _documento,
            "cuenta": _cuenta, "valor": int.parse(_valor.text.replaceAll(",", "")),
            "observacion": _observacion.text, "registro": '0', "mascara": _mascara,
            "archivo0": archivo0, "archivo1": archivo1, "archivo2": archivo2, "archivo3": archivo3
      ...
      child: Column(
        children: <Widget>[
          Align(
            ...
            child: Text('    ' + widget.actual.agenda.toString(), style: Theme.of(context).textTheme.overline ), ),
          Align(
            alignment: Alignment.bottomLeft,
            child: Text('    ' + widget.actual.agenda.toString(), style: Theme.of(context).textTheme.overline ), ),
          Align(
            alignment: Alignment.bottomLeft,
            child: Text('    ' + widget.actual.agenda.toString(), style: Theme.of(context).textTheme.overline ), ), // caption

          SizedBox(height: 10.0),

          Align(
            alignment: Alignment.bottomLeft,
            child: Text('  Medidor Nro.: ' + widget.actual.valor.toString(), style: Theme.of(context).textTheme.headline6 ), ),
          Align(
            alignment: Alignment.bottomLeft,
            child: Text('  Lectura anterior: ' + widget.actual.valor.toString().replaceAll(".", ","), style: Theme.of(context).textTheme.headline6 ), ),
        ],
      ),
    );
...
//          _adicionalFocus.unfocus();
        ...
        onTap: () async {
          var data = Data(
              ...
              descripcion: _desAgenda.text.toString(),  // Es usado para el retornar la descripción
              parData0: '',
              parData1: '',
              parData2: '',
              parData3: ''
          ...
          final result = await Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => RecordPage(dataRegistro: data,) ),
          )as Data;
          setState(() {
            ...
            if ( result.parData3.indexOf('.') > 0 )
              _valor.text = result.parData3.toString().substring(0, result.parData3.indexOf('.'));
            else _valor.text = '0';

          });

          late Record control;  // Averigua descripcion de cuenta (10343), primer presupuesto (10327) OJO puede que no Exista y auxiliar (10320), además cuando es actividad hereda 0,0,0
          control = await retrieveBusqueda('registro, descripcion', 'tabla = 10343 and registro = ' + _cuenta.toString() ) as Record;
          setState(() {
          ...
          control = await retrieveBusqueda('min(registro) AS registro, descripcion','tabla = 10327 and parametro0 = ' + _agenda.toString() ) as Record;
          setState(() {
            if (control.registro != null) {
          ...
          control = await retrieveBusqueda('min(registro) AS registro, descripcion','tabla = 10320 and parametro0 = ' + _parAgenda1.toString() ) as Record;
          setState(() {
            if (control.registro != null) {
      ...
      child: TextFormField(
        ...
        onTap: () async {
          var data = Data(
              titulo: 'Presupuesto',
              llave: _mascara,
              condicion: "(tabla = 10327 and parametro0 = '" + _agenda.toString() + "') or (tabla = 10327 and registro = 0)", // De la tabla de presupuesto de agenda actual
      ...
      child: TextFormField(
        ...
        onTap: () async {
          var data = Data(
              titulo: 'Auxiliar',
              llave: _documento,
              condicion: "(tabla = 10320 and parametro0 ='" + _parAgenda1.toString() + "') or (tabla = 10320 and registro = 0)", // and parametro0 ='" + _parametro1 + "'",
...
//            imageHeader: AssetImage("assets/images/icon.png"),
            description: "Diligencie un formulario por cada transacción, toda la información deberá ser completada",
            initialDate: DateTime.parse(_fecha.text.toString()), // String --> Date
...
class Data {
  ...
  String parData3;
  Data({
    required this.titulo, required this.condicion,
    required this.llave, required this.descripcion,
    required this.parData0, required this.parData1,
    required this.parData2, required this.parData3});
}
...
class DataDescripcion {
  ...
  String descripcion;
  DataDescripcion({
    required this.tabla,  required this.descripcion});
}
